...
/**
 * Mesure la santé de la file de mail pour les notifications qui conditionnent
 * l'accès à un compte (réinitialisation de mot de passe, invitation).
 *
 * Extrait de `SurveillerFileMailCommand` (mibeko-dashboard#60) pour que la
 * commande CLI et la console `/admin/sante` (mibeko-dashboard#110) partagent
 * la même définition d'« échec » et de « blocage » plutôt que de la dupliquer.
 *
...
class MailQueueHealthChecker
{
    ...
    /**
     * Jobs restés BLOQUÉS dans `jobs` au-delà d'un délai raisonnable — le
     * worker `mibeko-queue` ne tourne probablement plus. Ce cas n'écrit jamais
     * dans `failed_jobs` : sans ce contrôle, un worker arrêté serait invisible
     * tant qu'aucune tentative n'échoue.
     *
